"""
Tests for RansomlookCollector.

Unit tests cover parse_posts() with known API response shape (no HTTP).
Integration tests use pytest-httpx to mock the actual HTTP calls made by
collect() and health_check().

Actual ransomlook.io /api/recent field reference (verified 2026-03):
  post_title, group_name, description, link (relative), discovered, magnet
"""
import pytest
from pytest_httpx import HTTPXMock

from app.collectors.ransomlook import RansomlookCollector
from app.collectors.base import RawDetection


# ---------------------------------------------------------------------------
# Sample data that mirrors the real ransomlook.io API shape
# ---------------------------------------------------------------------------

SAMPLE_POST = {
    "post_title": "jenningsk12.org",
    "discovered": "2026-03-24 21:39:57.433444",
    "description": "School district data exfiltrated.",
    "link": "/blog/disclosures/69c2e4b58f1d14b743698da2",
    "magnet": None,
    "screen": "screenshots/inc ransom/jenningsk12org.png",
    "group_name": "inc ransom",
}

SAMPLE_POST_MINIMAL = {
    "post_title": "",
    "description": "",
    "link": "",
    "group_name": "",
}


# ---------------------------------------------------------------------------
# Fixtures
# ---------------------------------------------------------------------------

@pytest.fixture
def collector():
    return RansomlookCollector()


# ---------------------------------------------------------------------------
# Metadata tests
# ---------------------------------------------------------------------------

def test_collector_name(collector):
    assert collector.name == "ransomlook"


def test_collector_poll_interval(collector):
    assert collector.poll_interval_seconds == 600


# ---------------------------------------------------------------------------
# parse_posts unit tests — pure function, no HTTP
# ---------------------------------------------------------------------------

def test_parse_posts_returns_raw_detections(collector):
    results = collector.parse_posts([SAMPLE_POST])
    assert len(results) == 1
    assert isinstance(results[0], RawDetection)


def test_parse_posts_maps_post_title_to_title(collector):
    results = collector.parse_posts([SAMPLE_POST])
    assert results[0].title == "jenningsk12.org"


def test_parse_posts_maps_group_name_to_actor(collector):
    results = collector.parse_posts([SAMPLE_POST])
    assert results[0].actor == "inc ransom"


def test_parse_posts_builds_absolute_source_url(collector):
    """Ransomlook returns relative links — collector should make them absolute."""
    results = collector.parse_posts([SAMPLE_POST])
    expected = "https://www.ransomlook.io/blog/disclosures/69c2e4b58f1d14b743698da2"
    assert results[0].source_url == expected


def test_parse_posts_empty_link_gives_empty_source_url(collector):
    post = {**SAMPLE_POST, "link": ""}
    results = collector.parse_posts([post])
    assert results[0].source_url == ""


def test_parse_posts_source_is_collector_name(collector):
    results = collector.parse_posts([SAMPLE_POST])
    assert results[0].source == "ransomlook"


def test_parse_posts_content_includes_title(collector):
    results = collector.parse_posts([SAMPLE_POST])
    assert "jenningsk12.org" in results[0].content


def test_parse_posts_content_includes_description(collector):
    results = collector.parse_posts([SAMPLE_POST])
    assert "School district data" in results[0].content


def test_parse_posts_content_includes_group(collector):
    results = collector.parse_posts([SAMPLE_POST])
    assert "inc ransom" in results[0].content


def test_parse_posts_raw_payload_preserved(collector):
    results = collector.parse_posts([SAMPLE_POST])
    assert results[0].raw_payload == SAMPLE_POST


def test_parse_posts_empty_list(collector):
    assert collector.parse_posts([]) == []


def test_parse_posts_multiple_posts(collector):
    second = {**SAMPLE_POST, "post_title": "othervictim.com", "link": "/blog/disclosures/other"}
    results = collector.parse_posts([SAMPLE_POST, second])
    assert len(results) == 2
    assert results[1].title == "othervictim.com"


def test_parse_posts_empty_group_sets_actor_none(collector):
    """When group_name is empty string, actor should be None."""
    post = {**SAMPLE_POST, "group_name": ""}
    results = collector.parse_posts([post])
    assert results[0].actor is None


def test_parse_posts_minimal_post_does_not_raise(collector):
    """parse_posts should not raise even when optional fields are absent."""
    results = collector.parse_posts([SAMPLE_POST_MINIMAL])
    assert len(results) == 1
    assert results[0].title == ""
    assert results[0].actor is None


# ---------------------------------------------------------------------------
# collect() integration test — mocked HTTP
# ---------------------------------------------------------------------------

async def test_collect_returns_detections(collector, httpx_mock: HTTPXMock):
    """collect() hits the API and returns parsed RawDetection list."""
    httpx_mock.add_response(
        url="https://www.ransomlook.io/api/recent",
        json=[SAMPLE_POST],
    )
    results = await collector.collect()
    assert len(results) == 1
    assert results[0].title == "jenningsk12.org"


async def test_collect_empty_response(collector, httpx_mock: HTTPXMock):
    httpx_mock.add_response(
        url="https://www.ransomlook.io/api/recent",
        json=[],
    )
    results = await collector.collect()
    assert results == []


async def test_collect_raises_on_http_error(collector, httpx_mock: HTTPXMock):
    """collect() propagates HTTP errors via raise_for_status()."""
    httpx_mock.add_response(
        url="https://www.ransomlook.io/api/recent",
        status_code=503,
    )
    with pytest.raises(Exception):
        await collector.collect()


# ---------------------------------------------------------------------------
# health_check() tests — mocked HTTP
# ---------------------------------------------------------------------------

async def test_health_check_returns_true_on_200(collector, httpx_mock: HTTPXMock):
    httpx_mock.add_response(
        url="https://www.ransomlook.io/api/groups",
        status_code=200,
        json=[],
    )
    assert await collector.health_check() is True


async def test_health_check_returns_false_on_error(collector, httpx_mock: HTTPXMock):
    httpx_mock.add_response(
        url="https://www.ransomlook.io/api/groups",
        status_code=500,
        json={},
    )
    assert await collector.health_check() is False
